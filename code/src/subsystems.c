...
void linearGear(){
	...
	else if(joystickGetDigital(2, 6, JOY_DOWN) || 0){
...
void chainBar(){
	if(joystickGetDigital(2, 5, JOY_UP) || 0){
		smartMotorSet(MOTORS_CHAINB, 70);
	}
	else if(joystickGetDigital(2, 5, JOY_DOWN) || 0){
			smartMotorSet(MOTORS_CHAINB, 70);
	}
	else {
...
void coneGrabber(){
	...
	else if(joystickGetDigital(2, 8, JOY_DOWN) || 0){
			smartMotorSet(MOTOR_CONEG, -90);
	}
	else {
